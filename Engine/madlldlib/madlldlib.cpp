...
extern "C" __declspec(dllexport) int __stdcall CbMpegAudioDecoder(	const char *InFN, 
																	const char *OutFN,
																	int WavPad,
																	char *StatMsg,	
																	bool (__stdcall *CbFunc)(unsigned long FrameCnt, unsigned long InByteTot, struct mad_header *Header) )
{
	...
	int					wavflg = 1;	bstdfile_t *BstdFile;
	...
	do
	{
		 ...
		 */
		cvkill = CbFunc(FrameCount, ByteCount, &Frame.header);

		/* If cvkill is set to false, cancel.
		 */
		if (!cvkill) return(1);
	...
	if (WavPad == 1) 
	{
		...
		wavh.nBlockAlign		= (short)(wavh.wBitsPerSample / 8 * wavh.nChannels); //should be 2 for mono.
