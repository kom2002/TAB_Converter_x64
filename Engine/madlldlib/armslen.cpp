 ...
 * a status bar.
 */
bool __stdcall mycb (unsigned long fcnt, unsigned long bcnt, struct mad_header *mhdr) {
	...
	if (fcnt == 1) {
		...
		/*
		 * This checks for kill status from client,
		 * and returns false if there is a kill status
		 */
		rslt = ReadFile(ha, &pmsg, 256, &bamt, NULL);	
		if (rslt && (strcmp(&pmsg[0], "k")==0)) return false;
	}
